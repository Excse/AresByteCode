...
void ares::ClassWriter::visitClass(ares::ClassFile &classFile) {
    ...
    ares::writeU16(classFile.m_AccessFlags, m_ByteCode, m_Size, m_Offset);
    ares::writeU16(classFile.m_ThisClass, m_ByteCode, m_Size, m_Offset);
    ares::writeU16(classFile.m_SuperClass, m_ByteCode, m_Size, m_Offset);

    ares::writeU16(classFile.m_InterfacesCount, m_ByteCode, m_Size, m_Offset);
    for (auto index = 0; index < classFile.m_InterfacesCount; index++)
        ares::writeU16(classFile.m_Interfaces[index], m_ByteCode, m_Size, m_Offset);

    ares::writeU16(classFile.m_FieldsCount, m_ByteCode, m_Size, m_Offset);
    for (const auto &fieldInfo : classFile.m_Fields)
        ClassWriter::visitClassField(classFile, *fieldInfo);

    ares::writeU16(classFile.m_MethodCount, m_ByteCode, m_Size, m_Offset);
    for (const auto &methodInfo : classFile.m_Methods)
        ClassWriter::visitClassMethod(classFile, *methodInfo);

    ares::writeU16(classFile.m_AttributesCount, m_ByteCode, m_Size, m_Offset);
...
void ares::ClassWriter::visitClassCPInfo(ares::ClassFile &classFile,
    ...
    switch (info.m_Tag) {
        ...
        case ConstantPoolInfo::INTERFACE_METHOD_REF:
            ares::writeU16(info.m_Info.fieldMethodInfo.m_ClassIndex, m_ByteCode, m_Size, m_Offset);
